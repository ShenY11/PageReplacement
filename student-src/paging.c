...
void proc_init(pcb_t *proc) {
    ...
    memset(mem + PAGE_SIZE * freeFrame, 0, PAGE_SIZE);
    /*
    ...
    //frame_table[freeFrame].mapped = 1;  //TODO
    /*
...
uint8_t mem_access(vaddr_t address, char rw, uint8_t data) {
  ...
  /* If the entry is invalid, call page_fault() to read in and map a valid page for the page table entry. 
    ...
    fte_t *fte = frame_table + pte -> pfn; //TODO
    ...
    */
    paddr_t physAddr = (paddr_t) ((pte -> pfn * PAGE_SIZE) + offset);
    fte -> referenced = 1;
...
void proc_cleanup(pcb_t *proc) {
    ...
    for (size_t i = 0; i < NUM_PAGES; i++) {
        pte_t *temp_pte = pte + i;
        if (temp_pte -> valid == 1) {
            temp_pte -> valid = 0;
            frame_table[temp_pte -> pfn].mapped = 0;
            //temp_pte -> dirty = 0; //TODO-Proc_cleanup
        }
        if (swap_exists(temp_pte) == 1) {
            swap_free(temp_pte);
        }
    }
    ...
    frame_table[proc -> saved_ptbr].mapped = 0; //TODO
